...
2. **StudentRegistration.Application**: DTOs, Interfaces (service contracts), FluentValidation validators. Only `TokenService` implemented here; others are interfaces only.
3. **StudentRegistration.Infrastructure**: Business service implementations (`AuthService`, `StudentService`, `EnrollmentService`), EF Core `ApplicationDbContext`, Migrations, Data seeding.
4. **StudentRegistration.Api**: REST Controllers, DI configuration in `Program.cs`, Exception middleware.

**Key Pattern**: Services are **implemented in Infrastructure layer**, not Application. Application defines the interfaces; Infrastructure provides implementations.
...
- **Standalone Components**: All components are standalone with explicit imports; no NgModules. Uses Angular 21 standalone APIs.
- **SSR**: Server-Side Rendering support enabled via `@angular/ssr` for improved SEO and initial load performance.

## Critical Workflows
**Backend:**
- Start: `dotnet run` in `Backend/StudentRegistration.Api` → Auto-creates DB, applies migrations, seeds data.
- API runs on `http://localhost:5004` by default.
- Never run manual `dotnet ef database update`; the app handles migrations on startup automatically.
- To create a new migration: `dotnet ef migrations add MigrationName -p StudentRegistration.Infrastructure -s StudentRegistration.Api`

**Frontend:**
- Start: `npm start` (or `ng serve`) in `Frontend/student-registration` → Runs on port 4200.
- Build (with SSR): `npm run build` → Outputs to `dist/student-registration/`.
- Build (SSR static): Use `npm run serve:ssr:student-registration` to test SSR locally.
